...
def hashOpts(opts):
    ...
    gs = wlutil.checkGitStatus(buildroot_dir)
...
class Builder:
    ...
    def configure(self, env):
        ...
        wlutil.run(['make', 'defconfig'], cwd=buildroot_dir, env=env)
        shutil.copy(buildroot_dir / '.config', defconfig)

        kFrags = [defconfig, toolKfrag] + self.opts['configs']
        mergeScript = br_dir / 'merge_config.sh'
        wlutil.run([mergeScript] + kFrags, cwd=buildroot_dir, env=env)
        ...
        localMk = buildroot_dir / 'local.mk'
    ...
    def buildBaseImage(self, task, changed):
        ...
        try:
            ...
            wlutil.run(['make'], cwd=buildroot_dir, env=env)

            self.outputImg.parent.mkdir(parents=True, exist_ok=True)
            shutil.move(img_dir / 'rootfs.ext2', self.outputImg)

            self.outputDir.mkdir(parents=True, exist_ok=True)
            shutil.copy(buildroot_dir / '.config', self.outputDir / 'buildroot_config')
    ...
    def upToDate(self):
        return [wlutil.config_changed(wlutil.checkGitStatus(buildroot_dir)),
